...
house_facility = db.table(
    "i_house_facility",
    db.Column("house_id",db.Integer,db.ForeignKey("i_house.id"),primary_key=True), # 房屋编号
    db.Column("facility_id",db.Integer,db.ForeignKey("i_facility.id"),primary_key=True) # 设施编号
)
...
class Order(BaseModel,db.Model):
    """订单"""
    __tablename__ = "i_order"

    id= db.Column(db.Integer,primary_key=True) # 订单编号
    user_id = db.Column(db.Integer,db.ForeignKey("i_user.id"),nullable=True) # 下订单用户的编号
    house_id = db.Column(db.Integer,db.ForeignKey("i_house.id"),nullable=True) # 预定的房间编号
    begin_date= db.Column(db.DateTime,nullable=False) # 预定的起始日期
    end_data = db.Column(db.DateTime,nullable=False) # 预定的结束日期
